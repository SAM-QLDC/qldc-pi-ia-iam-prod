...
#options['Set Value Flag'] = 'SHP'							## String | blank | Flag used for fields set from data
#options['Default Value Flag'] = 'SHP'						## String | blank | Flag used for fields set from the default value column
...
nw.odic_import_ex(
'SHP',										# import data format => Arc ShapeFiles
'C:\Temp\SHPConfig.cfg',						# field mapping config file
options,									# specified options override the default options

# table group
'node',										# import to table name
'C:\Temp\test.SHP'							# import from file shapefile name
)
...
nw.commit('Date imported from SHP')
nw.unreserve
puts 'Committed'
